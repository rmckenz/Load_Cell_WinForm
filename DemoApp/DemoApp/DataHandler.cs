...
namespace DemoApp
{
    class DataHandler
    {
        ...
        public double[] movingArray(double values)
        {
            ...
            while(queue.Count() > 30) //10 is arbitrary - whatever size you want the moving array
            {
                queue.Dequeue();
            }
        ...
        public void storageArray(double[] movingArray, double currentValue)
        {
            ...
            if(Math.Abs(mAverage - currentValue) > 2) //TOSET th difference to trigger an append to the array is arbritray and should be reevaluated when load cell is observed
            {
        ...
        public void readData()
        {
            ...
            while (flag)
            {
                ...
                double rawValue = processData(SerialPort.sRead()); //Converting most recent reading to double and storing it as rawValues
                double[] mArray = movingArray(rawValue); //Adds rawValues to the queue array and removes values to keep it at a length of X
                storageArray(mArray, rawValue); //Using the void method to update valueArray
